...
# Trap SIGINT (Ctrl+C) and SIGTERM to execute the cleanup function
...
do
  ...
  token=$(echo $response | jq -r '.token');
  echo "Login diner..."
  curl -s -X POST $host/api/order -H 'Content-Type: application/json' -d '{"franchiseId":10, "storeId":6, "items":[{ "menuId": 1, "description": "Veggie", "price": 0.05 }]}'  -H "Authorization: Bearer $token" > /dev/null;
  echo "Bought a pizza..."
  sleep 15;
  curl -s -X DELETE $host/api/auth -H "Authorization: Bearer $token" > /dev/null;
