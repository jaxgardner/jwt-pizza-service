...
beforeAll(async () => { 
    ...
    await request(app).post('/api/franchise').send({
        name: 'Franchise 1',
        admins: [
            {
                email: admin.email,
...
describe("franchiseRouter", () => {
    ...
    test("delete franchise", async () => {
        ...
        const res = await request(app).delete('/api/franchise/2').set('Authorization', `Bearer ${authToken}`);
        expect(res.status).toBe(200);
    ...
    test("delete store", async () => {
        await request(app).post('/api/franchise/1/store').send({
            name: 'Store 2',
        }).set('Authorization', `Bearer ${authToken}`);

        const res = await request(app).delete('/api/franchise/1/store/2').set('Authorization', `Bearer ${authToken}`);
        expect(res.status).toBe(200);
